...
class Sidebar extends React.Component {
  ...
  fetchResults() {
    request.get("/search?q=" + this.state.query).end((err, res) => {
      if (err) {
        ...
        console.log(this.state.results.sites)
  ...
  render() {
    ...
    const results = this.state.results.sites || [];
    console.log(results)
    ...
    return (
      <div>
        ...
        {resultsCount > 0 ? (
          <div id="results-area">
            <h5>
               found <span className="highlight">{resultsCount}</span> sites in{" "}
            </h5>
            <ul> {listItems} </ul>
          </div>
        ) : null}
